...
//		pd_2=request.getParameter("confirmPassword");
		ad=request.getParameter("confirmAddress");
...
//		}
		...
		try{
			con=DriverManager.getConnection(uri,"root","root");
			String updateCondition="UPDATE Test.user SET address = '"+ad+"' WHERE logname = '"+logname+"' ";
			sql=con.createStatement();
			int m=sql.executeUpdate(updateCondition);
			if(m!=0){
				loginBean.setAddress(ad);
				String backnews="地址添加/修改成功";
				PrintWriter out = response.getWriter();
				out.write("<script language='javascript'>alert('"+backnews+"');window.location.href='user.jsp';</script>");
			}
			else{
				String backnews="警告：地址添加/修改失败！";
				PrintWriter out = response.getWriter();
				out.write("<script language='javascript'>alert('"+backnews+"');window.location.href='changeaddress.jsp';</script>");
			}
		}
		catch(SQLException exp){
			loginBean.setSuccess(false);
