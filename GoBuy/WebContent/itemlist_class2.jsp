...
<jsp:useBean id="show_2" class="JavaBean.ItemList" scope="session"/>
...
<html>
	<head>
		<base href="<%=basePath%>">    
    	<title>My JSP 'itemlist_class2.jsp' starting page</title>
		...
		<div style="width:1198px;length:544px;">
			...
			<BR>当前显示内容
				<table border=1>
					...
					<jsp:getProperty name="show_2" property="presentPageResult"/>
				</table>
				<BR>每页最多显示 <Font color=blue><jsp:getProperty name="show_2" property="pageSize"/></Font> 条商品信息
				<BR>当前显示第<Font color=blue>
					<jsp:getProperty name="show_2" property="showPage"/>
					</Font>页，共有
					<Font color=blue><jsp:getProperty name="show_2" property="pageAllCount"/>
					</Font>页。
				<BR>单击“上一页”或“下一页”按钮查看信息
				<table>
					<tr>
						<td>
							<form action="showlist_class2" method="post">
							<input type=hidden name="showPage" value="<%=show_2.getShowPage()-1 %>">
